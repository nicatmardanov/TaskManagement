...
; (function ($, window, undefined) {
	...
	$(document).ready(function () {
		...
		$.ajax({
			url: '/Task/NotificationsCount',
			type: 'get',
			success: function (res) {
				var notifications = res.split('-');
				var length = notifications.length;
				var tile_length = $('.tile-stats-custom').length;
				for (var i = 0; i < length; i++) {
					if (tile_length > 0) {
						$($($('.tile-stats-custom')[i]).find('span')).html(notifications[i]);
						//$($($('.tile-stats-custom')[i]).find('span')).attr('data-end', notifications[i]);
						$($($('.tile-stats-custom')[i]).find('span')).html(notifications[i]);
					}
					$($($('.sidebar_ntfcts li')[i]).find('.badge')).html(notifications[i]);
					$($($('.top_ntfcts li')[length - i - 1]).find('.badge')).html(notifications[i]);

				}

				//$(".tile-stats-custom").each(function (i, el) {
				//	var $this = $(el),
				//		$num = $this.find('.num'),
